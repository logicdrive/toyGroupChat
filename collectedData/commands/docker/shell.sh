...
echo "docker run --name ${DOCKER_IMAGE_NAME}_con -e SPRING_PROFILES_ACTIVE=docker -p $DOCKER_IMAGE_PORT:8080 --rm -it --entrypoint /bin/sh $DOCKER_USER_NAME/$DOCKER_IMAGE_NAME:$DOCKER_VERSION"